 ...
 */
function wp_slider_wp_trim_words( $text, $num_words = 55, $more = null ) {
	if ( null === $more ) {
		$more = __( '&hellip;' );
	}
...
function wp_slider_tags_allow( $allowedposttags ) {
	...
	$allowedposttags = apply_filters( 'wp_slider_custom_tags_allow', $allowedposttags );
	
	return $allowedposttags;
}

function wp_slider_css_allow($allowed_attr) {
	...
	$allowed_attr = apply_filters( 'wp_slider_custom_css_allow', $allowed_attr );
	
	return $allowed_attr;
}

function wp_slider_wp_check_filetype_and_ext($data, $file, $filename, $mimes) {
