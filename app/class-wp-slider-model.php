...
require_once 'wp-slider-functions.php';

class WP_Slider_Model {
	...
	function get_upload_path() {
		...
		return $uploads['basedir'] . '/wp-slider/';
	}
	
	function get_upload_url() {
	
		$uploads = wp_upload_dir();
		return $uploads['baseurl'] . '/wp-slider/';
	}
	...
	{				
		...
		add_filter( 'wp_check_filetype_and_ext', 'wp_slider_wp_check_filetype_and_ext', 10, 4);
		...
		remove_filter( 'wp_check_filetype_and_ext', 'wp_slider_wp_check_filetype_and_ext');
	...
	{
		if ( !check_admin_referer('wp-slider', 'wp-slider-export') || !isset($_POST['allsliders']) || !isset($_POST['sliderid']) || !is_numeric($_POST['sliderid']) )
			exit;
		...
		header("Content-Disposition: attachment; filename=wp_slider_export.xml");
	...
	function generate_body_code($id, $has_wrapper, $atts) {
		...
		if ($item_row != null)
		{
			...
			$ret .= '<div class="wpslider-container" id="wpslider-container-' . $id . '"'; 
			...
			// div data tag
			$ret .= '<div class="wpslider' . ($has_woocommerce ? ' woocommerce' : '') . '" id="wpslider-' . $id . '" data-sliderid="' . $id . '" data-width="' . $data->width . '" data-height="' . $data->height . '" data-skin="' . $data->skin . '"';
			...
			{
				$ret .= '<script>jQuery(document).ready(function(){jQuery(".wp-engine").css({display:"none"});jQuery(".wpslider").wpslider({forceinit:true});});</script>';
			}
	...
	function get_post_items($options) {
		...
		foreach($posts as $post)
		{
			...
			$excerpt = wp_slider_wp_trim_words($excerpt, $options->excerptlength);
	...
	function replace_custom_field($postdata, $postmeta, $field, $textlength) {
		...
		$postdata = apply_filters( 'wp_slider_custom_post_field_content', $postdata );
			...
			foreach($matches[1] as $match)
			{
				...
				if (array_key_exists($match, $postdata))
				{
					...
					if ($match == 'post_content' || $match == 'post_excerpt')
						$replace = wp_slider_wp_trim_words($replace, $textlength);
				}
	...
	function get_settings() {
		...
		$userrole = get_option( 'wp_slider_userrole' );
		if ( $userrole == false )
		{
			update_option( 'wp_slider_userrole', 'manage_options' );
			$userrole = 'manage_options';
		}
		
		$keepdata = get_option( 'wp_slider_keepdata', 1 );
		
		$disableupdate = get_option( 'wp_slider_disableupdate', 0 );
		...
		$usepostsave = get_option( 'wp_slider_usepostsave', 0 );
		
		$addextrabackslash = get_option( 'wp_slider_addextrabackslash', 0 );
		
		$thumbnailsize = get_option( 'wp_slider_thumbnailsize' );
		if ( $thumbnailsize == false )
		{
			update_option( 'wp_slider_thumbnailsize', 'large' );
			$thumbnailsize = 'large';
		}
		
		$jetpackdisablelazyload = get_option( 'wp_slider_jetpackdisablelazyload', 1 );

		$supportmultilingual = get_option( 'wp_slider_supportmultilingual', 1 );
	...
	function save_settings($options) {
		...
		if (!isset($options) || !isset($options['userrole']))
			...
			$userrole = 'manage_options';
		update_option( 'wp_slider_userrole', $userrole );
		
		if (!isset($options) || !isset($options['keepdata']))
			$keepdata = 0;
		else
			$keepdata = 1;
		update_option( 'wp_slider_keepdata', $keepdata );
		
		if (!isset($options) || !isset($options['disableupdate']))
			$disableupdate = 0;
		else
			$disableupdate = 1;
		update_option( 'wp_slider_disableupdate', $disableupdate );
		
		if (!isset($options) || !isset($options['supportwidget']))
			$supportwidget = 0;
		else
			$supportwidget = 1;
		update_option( 'wp_slider_supportwidget', $supportwidget );
		
		if (!isset($options) || !isset($options['addjstofooter']))
			$addjstofooter = 0;
		else
			$addjstofooter = 1;
		update_option( 'wp_slider_addjstofooter', $addjstofooter );
		
		if (!isset($options) || !isset($options['jsonstripcslash']))
			$jsonstripcslash = 0;
		else
			$jsonstripcslash = 1;
		update_option( 'wp_slider_jsonstripcslash', $jsonstripcslash );
		
		if (!isset($options) || !isset($options['usepostsave']))
			$usepostsave = 0;
		else
			$usepostsave = 1;
		update_option( 'wp_slider_usepostsave', $usepostsave );
		
		if (!isset($options) || !isset($options['addextrabackslash']))
			$addextrabackslash = 0;
		else
			$addextrabackslash = 1;
		update_option( 'wp_slider_addextrabackslash', $addextrabackslash );
		
		if (isset($options) && isset($options['thumbnailsize']))
			$thumbnailsize = $options['thumbnailsize'];
		else
			$thumbnailsize = 'large';
		update_option( 'wp_slider_thumbnailsize', $thumbnailsize );

		if (!isset($options) || !isset($options['jetpackdisablelazyload']))
			$jetpackdisablelazyload = 0;
		else
			$jetpackdisablelazyload = 1;
		update_option( 'wp_slider_jetpackdisablelazyload', $jetpackdisablelazyload );

		if (!isset($options) || !isset($options['supportmultilingual']))
			$supportmultilingual = 0;
		else
			$supportmultilingual = 1;
		update_option( 'wp_slider_supportmultilingual', $supportmultilingual );
	}
	
	function get_plugin_info() {
	
		$info = get_option('wp_slider_information');
		if ($info === false)
			return false;
	...
	function save_plugin_info($info) {
		...
		update_option( 'wp_slider_information', serialize($info) );
	}
	...
	function check_license($options) {
		...
		if ( !isset($options) || empty($options['wp-slider-key']) )
		{
			return $ret;
		}
	
		$key = sanitize_text_field( $options['wp-slider-key'] );
		if ( empty($key) )
			return $ret;
