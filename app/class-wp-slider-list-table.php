...
class WP_Slider_List_Table extends WP_List_Table {
	...
	function get_columns()
	{
		$columns = array(
				...
				'id' => __('ID', 'wp_slider'),
				'name' => __('Name', 'wp_slider'),
				'shortcode' => __('Shortcode', 'wp_slider'),
				'phpcode' => __('PHP code', 'wp_slider'),
				'time' => __('Created', 'wp_slider')
		);
		return $columns;
	}
	...
	{
		$table_nonce = wp_create_nonce( 'wp-list-table-nonce' );
		...
		switch( $column_name ) {
			case 'id':
				...
				{
					$actions = array(
							'edit' => sprintf('<a href="?page=%s&itemid=%s">Edit</a>', 'wp_slider_edit_item', $item['id']),
							...
							'view' => sprintf('<a href="?page=%s&itemid=%s">View</a>', 'wp_slider_show_item', $item['id'])
					);
				}
			...
			case 'shortcode':
				return esc_attr('[wp_slider id=' . $item['id'] . ']');
			case 'phpcode':
				return esc_attr('<?php echo do_shortcode(\'[wp_slider id=' . $item['id'] . ']\'); ?>');
			default:
				return $item[ $column_name ];
		}
	}
	...
	function get_views(){
		...
		// All
		$all_url = admin_url('admin.php?page=wp_slider_show_items');
		$class = ($current == 'all' ? ' class="current"' :'');
		$views['all'] = "<a href='" . $all_url . "' " . $class . " >All (" . $this->published . ")</a>";
	
		// Published
		$publish_url = admin_url('admin.php?page=wp_slider_show_items&item_status=publish');
		$class = ($current == 'publish' ? ' class="current"' :'');
		$views['publish'] = "<a href='" . $publish_url . "' " . $class . " >Published (" . $this->published . ")</a>";
	
		// Trash
		$trash_url = admin_url('admin.php?page=wp_slider_show_items&item_status=trash');
